...
def test_embeddings_prepared_for_other_classes_are_refused(tmp_path):
    rng = np.random.default_rng(0)
    emb = tmp_path / "embeddings"
    emb.mkdir()
    for split, n in (("train", 30), ("validation", 9)):
        x, y = _separable(n, rng)  # labels 0-2: prepared for the 3-class model
        np.savez(emb / f"{split}.npz", embeddings=x, labels=y)
    cfg = load_config()
